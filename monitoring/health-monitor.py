...
class HealthMonitor:
    ...
    def _parse_services(self) -> Dict[str, Tuple[str, int]]:
        """Parse SERVICES env var: db:5432,redis:6379,..."""
        services = {}
        services_str = os.getenv('SERVICES', 'db:5432,redis:6379')
        for service_def in services_str.split(','):
            parts = service_def.strip().split(':')
            if len(parts) == 2:
                name, port = parts[0].strip(), parts[1].strip()
                try:
                    services[name] = (name, int(port))
                except ValueError:
                    logger.error(f"Invalid port for {name}: {port}")
        return services
