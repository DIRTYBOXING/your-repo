...
const server = http.createServer((req, res) => {
  if (req.url === "/healthz") {
    const active = [...connectionsByUser.values()].reduce(
      (sum, set) => sum + set.size,
      0,
    );
    res.setHeader("content-type", "application/json");
    res.end(JSON.stringify({ status: "ok", activeConnections: active }));
    return;
  }
...
wss.on("connection", (ws, req) => {
  ...
  ws.on("message", (raw) => {
    ...
    if (payload.type === "message" && payload.recipientId) {
      ...
      const delivered = fanout(payload.recipientId, envelope);
